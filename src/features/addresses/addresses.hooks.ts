...
export const useAddresses = (addressId?: string) => {
  ...
  const { data, isError: isAddressesError, error: addressesError, isLoading: isAddressesLoading } = useQuery({
    queryKey: ['addresses'],
    queryFn: getAddresses,
  });

  const { mutate: createAddressMutate, isPending: isCreatingAddress, isError: isCreatingAddressError, error: creatingAddressError } = useMutation<ApiSuccess<Address>, AxiosError<ApiError>, CreateAddressBody>({
  ...
  const { mutate: updateAddressMutate, isPending: isUpdatingAddress, isError: isUpdatingAddressError, error: updatingAddressError } = useMutation<ApiSuccess<Address>, AxiosError<ApiError>, UpdateAddressBody>({
  ...
  const { mutate: deleteAddressMutate, isPending: isDeletingAddress, isError: isDeletingAddressError, error: deletingAddressError } = useMutation<ApiSuccess<null>, AxiosError<ApiError>, string>({
