...
export default function OrderDetailPage() {
  ...
  if (isOrderLoading) return <p className="container py-20 text-center">Loading...</p>;
  if (!order)    return (
    <div className="container py-20 text-center">
      <p>Order not found</p>
      <Link to="/orders" className="underline text-sm mt-2 block">Back to orders</Link>
    </div>
  );
    ...
    <div className="container py-8 max-w-3xl">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_COLORS[order.data?.status || "pending"]}`}>
            {order.data?.status}
          </span>
          {canCancel && (
            <button
            ...
            {STATUS_STEPS.map((step, i) => (
              <div key={step.key} className="flex items-center flex-1 last:flex-none">
                ...
                {i < STATUS_STEPS.length - 1 && (
                  <div className={`h-0.5 flex-1 mx-1 ${i < currentStep ? "bg-black" : "bg-gray-200"}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              {(order.data?.discount ?? 0) > 0 && (
                <div className="flex justify-between text-green-600">
                  <span>
                    Discount
                    {order.data?.promo_code && ` (${order.data?.promo_code.code})`}
                  </span>
              ...
              <div className="flex justify-between">
                ...
                <span>{order.data?.shipping === 0 ? "Free" : `${order.data?.shipping} EGP`}</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="border rounded-lg p-4">
            ...
            <div className="text-sm text-muted-foreground space-y-0.5">
              <p className="font-medium text-foreground">{order.data?.address.full_name}</p>
              <p>{order.data?.address.address_line}</p>
              <p>{order.data?.address.city}{order.data?.address.state && `, ${order.data?.address.state}`}</p>
          ...
          <div className="border rounded-lg p-4">
            ...
            <div className="text-sm text-muted-foreground space-y-0.5">
              ...
              {order.data?.shipping_method.carrier && <p>{order.data?.shipping_method.carrier}</p>}
