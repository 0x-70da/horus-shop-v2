...
export const useCart = () => {
  ...
  >({
    ...
    mutationFn: ({ itemId, variantId = null, quantity }) =>
      addToCart(itemId, variantId, quantity),
    onSuccess: () => {
  ...
  const {
    ...
    isError: isUpdatingCartItemError,
    error: updateCartItemError,
  } = useMutation<
    ApiSuccess<CartItem | undefined>,
    AxiosError<ApiError>,
  ...
  const {
    ...
    isError: isRemovingFromCartError,
    error: removeFromCartError,
  } = useMutation<ApiSuccess, AxiosError<ApiError>, { itemId: string }>({
  ...
  const {
    ...
    isError: isClearingCartError,
    error: clearCartError,
  } = useMutation<ApiSuccess, AxiosError<ApiError>>({
